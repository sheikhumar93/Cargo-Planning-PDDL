...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for node in self.a_levels[level-1]:
            effnode_set = node.effnodes
            if len(self.s_levels) <= level:
                self.s_levels.append(set())
            for effnode in effnode_set:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_a1 in node_a1.action.effect_add:
            for effect_a2 in node_a2.action.effect_rem:
                if effect_a1 == effect_a2:
                    return True

        for eff_a1 in node_a1.action.effect_rem:
            for eff_a2 in node_a2.action.effect_add:
                if eff_a1 == eff_a2:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for add_effect_a1 in node_a1.action.effect_add:
            for pos_precon_a1 in node_a2.action.precond_pos:
                if add_effect_a1 == pos_precon_a1:
                    return True

        for neg_effect_a1 in node_a1.action.effect_rem:
            for pos2_precon_1 in node_a2.action.precond_pos:
                if neg_effect_a1 == pos2_precon_1:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # explanation taken from the Course Forums: https://discussions.udacity.com/t/problem-with-test-competing-needs-mutex/227344/2
        for parent in node_a1.parents:
            for parent_a2 in node_a2.parents:
                if parent.is_mutex(parent_a2):
                    return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        problem_goals = list(self.problem.goal)
        for indx, level in enumerate(self.s_levels):
            if not problem_goals:
                break
            for state in level:
                if state.symbol in problem_goals:
                    problem_goals.remove(state.symbol)
                    level_sum += indx
                    break
        return level_sum
